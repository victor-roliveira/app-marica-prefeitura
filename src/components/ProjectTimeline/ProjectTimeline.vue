    ...
    <v-card class="pt-card" elevation="2">
        <v-card-text class="pt-root">
            ...
            <TimelineCanvas :project="data.project" :stages="sortedStages" :progress="data.progress"
                :milestones="data.milestones" :alterarions="data.alterations" :config="data.config" />
        </v-card-text>
    </v-card>
</template>
...
const progressByStage = computed < Record < string, StageProgress | undefined >> (() => {
    ...
    for (const p of props.data.progress) {
        map[p.step_id] = p;
    }
    return map;
});
...
const showEmojis = computed(() => {
    return props.data.config?.show_icons_checked ?? true;
});
